"""
Chapter 2 Intelligent Agents - Student Starter
...
class Percept:
    """Current sensor content supplied to the agent.

    TODO(CH2-F): Refactor this percept so it explicitly supplies the
    current direction, released ghost positions, and frightened-mode
    status or remaining time. Preserve the useful existing fields.
    """
...
class PacManAgent:
    def __init__(self, game: "Game"):
        ...
        # TODO(CH2-G): Add compact internal state derived from the
        # percept sequence, such as visit counts or a short position
        # history. Initialize named utility weights here as well.
    ...
    def choose_action(self, percept: Percept) -> tuple[int, int]:
        ...
        # TODO(CH2-F): This is dynamic back-channel access to the live
        # environment. Move released ghost positions into Percept and
        # use the percept field here instead.
        active_ghosts = {
            ghost.position for ghost in self.game.ghosts if ghost.released
        }
        ...
        for action in percept.legal_actions:
            ...
            pellet_distance = self.shortest_distance(next_position, target_pellets)
            ghost_distance = self.shortest_distance(next_position, active_ghosts)

            # TODO(CH2-H): Replace unexplained numeric literals with
            # descriptive named weights and separate evaluation from
            # selection. Add a memory-based revisit contribution.
            utility = -2.0 * pellet_distance
            ...
            # TODO(CH2-F): Frightened status is dynamic sensor data.
            # Receive it through Percept rather than self.game.
            if self.game.ghosts_are_frightened():
                if next_position in active_ghosts:
                    utility += 500.0
                else:
                    utility -= ghost_distance * 7.0
            else:
                if next_position in active_ghosts:
                    utility -= 1000.0
                    ...
                    utility += min(ghost_distance, 8)

            # TODO(CH2-F): Current direction should also arrive through
            # Percept rather than through the live Game object.
            if action == self.game.player.direction:
                utility += 1.0

            if utility > best_utility:
                best_utility = utility
                best_action = action
                best_details = (pellet_distance, ghost_distance)
        ...
        self.last_reason = (
            ...
            f"pellet={best_details[0]} | ghost={best_details[1]}"
        )
...
class Game:
    ...
    def sense_environment(self) -> Percept:
        # TODO(CH2-F): Expand this sensor interface so every dynamic
        # fact used by choose_action is explicit in the Percept.
        return Percept(
