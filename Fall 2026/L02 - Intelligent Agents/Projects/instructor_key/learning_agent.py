...
class LearningAgent:
    ...
    def propose_new_weights(self) -> UtilityWeights:
        """Problem generator. Return ``self.best_weights`` with a small
        random nudge applied to one or two fields. Do not mutate any
        state here and do not assign the result anywhere -- the training
        harness decides whether and when to actually try it, by setting
        ``self.performance_element.weights`` to what this returns."""
        fields = list(self.best_weights.__dataclass_fields__)
        chosen = self.rng.sample(fields, k=min(2, len(fields)))

        updates = {}
        for name in chosen:
            current = getattr(self.best_weights, name)
            magnitude = max(abs(current), 1.0)
            nudge = self.rng.gauss(0.0, self.PERTURBATION_STRENGTH * magnitude)
            updates[name] = current + nudge

        return replace(self.best_weights, **updates)

    def learn(self, performance: float) -> None:
        """Critic + learning element. Called once after each training
        episode with that episode's performance measure -- the critic's
        judgement, computed by the environment, not by you. The weights
        that produced this episode are sitting in
        ``self.performance_element.weights`` right now.

        If they beat ``self.best_performance``, keep them as the new best.
        Otherwise, revert ``self.performance_element.weights`` to
        ``self.best_weights`` so a bad experiment does not stick around
